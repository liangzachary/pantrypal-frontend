...
export default function RewardScreen() {
  ...
  // localStorage key for "has unlocked recipe #2"
  const PROGRESS_KEY = "breakfastUnlocked";

  const handleContinue = () => {
    // ✅ Only unlock the next node (recipe #2). Do NOT increment past 1.
  ...
  const LIFT      = 50;  // px move star + mascot + text up

  return (
    <div
      className="max-w-[480px] mx-auto bg-gradient-to-b from-[#4FB9B0] to-[#87CEEB] relative overflow-hidden flex flex-col items-center"
    ...
    >
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) { .reward-spin,.reward-bob { animation: none !important; } }
      `}</style>

      {/* Wrapper that lifts everything visually */}
      <div
      ...
      {/* CTA button (stays pinned to bottom) */}
